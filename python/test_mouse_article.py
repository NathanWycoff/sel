...
betas = np.array([0.01 for i in range(H)])

THETA_in = mult / np.sqrt(P) * np.random.normal(size=[H,P]) 
...
for epoch in tqdm(range(epochs)):
    ...
    # Decrease learning rate one third and two thirds of the way through
    if epoch == onethird or epoch == twothirds:
        opt.learn_rate /= 2.
