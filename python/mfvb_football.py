...
snn = ALifNeuralNetwork(R, H, P, Q, t_eps = t_eps, mb_size = 1, costfunc = mse, optimizer = opt, thresh = 1)
...
THETA_in_draws = [np.random.normal(loc = LAMBDA_in, scale = np.abs(PHI_in)) for _ in range(V)]
THETA_rec_draws  = [np.random.normal(loc = LAMBDA_rec, scale = np.abs(PHI_rec)) for _ in range(V)]
THETA_out_draws  = [np.random.normal(loc = LAMBDA_out, scale = np.abs(PHI_out)) for _ in range(V)]

yv = np.empty([V,N])
for n in tqdm(range(N)):
    # Define inputs for this datum
    def get_xt(self, ip, ti):
        xt = X_vec[n].reshape([P])
        return xt
    target = np.repeat(y_vec[n_of_epoch], t_steps)
...
for v in range(V):
    plt.scatter(x_val, yv[v,:], color = 'orange', alpha = 0.5)
plt.title("Data")
plt.subplot(2,1,2)
plt.title("Data")
plt.plot(mb_costs)
